...
export default function() {
    ...
    const getHead = (
        ...
        return computed(() => {
            ...
            do {
                ...
                mappings = data.value.canPassRed ? fallbackFilter(mappings, x => !!x.canPass) : mappings
                mappings = data.value.slowTrack && data.value.blocksClear > 1 ? filter(mappings, x => !!x.slow) : mappings
                mappings = data.value.gradeTimerOn && data.value.blocksClear != 1 ? filter(mappings, x => !!x.grade || !!x.timer) : mappings
                if (!mappings.length) {
                    blocksClear--
                }
    ...
    const filter = (source: Uniform5SignalMapping[], predicate: Uniform5SignalMappingPredicate) => {
        const result = source.filter(predicate)
        return result
    }

    const fallbackFilter = (source: Uniform5SignalMapping[], predicate: Uniform5SignalMappingPredicate) => {
        const result = source.filter(predicate)
